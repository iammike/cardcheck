#!/usr/bin/env node
/**
 * Test runner for CardCheck extension
...
function extractCardData(testCase) {
  ...
  if (data.title && (!data.name || !data.year || !data.set || !data.number)) {
...
async function runTests() {
  console.log('CardCheck Test Runner');
  console.log('=====================\n');
  console.log(`Running ${testCases.cases.length} test cases...\n`);
  ...
  for (const testCase of testCases.cases) {
    ...
    if (actualSite !== expectedSite) {
      console.log('FAIL (wrong routing)');
      failures.push({
        name: testCase.name,
    ...
    if (searchResult.error) {
      ...
      failures.push({ name: testCase.name, reason: searchResult.error });
      failed++;
      continue;
    }
    ...
    if (validation.skip) {
      // No expected result defined, just check we got results
      console.log(`PASS (${searchResult.exactMatch ? 'exact match' : searchResult.resultCount + ' results'}) [no validation]`);
      skipped++;
      passed++;
    } else if (validation.found) {
      console.log(`PASS - found "${validation.match.name}" in ${validation.match.category}`);
      passed++;
    } else {
      console.log(`FAIL (expected "${expectedName}" not found)`);
      failures.push({
        name: testCase.name,
